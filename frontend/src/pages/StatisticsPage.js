...
const StatisticsPage = () => {
    ...
    const { data: networkStatsData, isLoading: networkStatsLoading } = useQuery({
        queryKey: ['network', 'stats'],
        queryFn: networkAPI.getNetworkStats,
    });

    if (summaryLoading || distributionsLoading || confidenceLoading || networkStatsLoading) {
        return (
    ...
    // Prepare chart data based on actual database statistics
    ...
    const methodOverlapData = [
        ...
        { methods: 'Orthology based', overlaps: 38, description: 'Cross-validated predictions' },
    ];
    ...
    // Prepare network size distribution by method (combine normal and predicted)
    const methodDistribution = networkStatsData?.data?.method_distribution || [];
    const combinedMethodCounts = {};
    methodDistribution.forEach(({ method_name, method_type, interaction_count }) => {
        // Remove 'predicted_' prefix and normalize case for grouping
        const baseName = method_name.replace(/^predicted_/i, '').toLowerCase();
        if (!combinedMethodCounts[baseName]) {
            combinedMethodCounts[baseName] = 0;
        }
        combinedMethodCounts[baseName] += parseInt(interaction_count);
    });
    const networkSizeData = Object.entries(combinedMethodCounts).map(([name, value], idx) => ({
        ...
        <Container maxWidth="lg">
            <Box sx={{ py: 4 }}>
                ...
                >
                    ...
                    {/* Top Interactors and Network Size Distribution */}
                    <Grid container spacing={3} sx={{ mb: 4 }} justifyContent="center" alignItems="stretch">
                        <Grid item xs={12} md={7}>
                            <Card sx={{ height: '100%', width: '100%' }}>
                                <CardContent>
                                    <Typography variant="h6" gutterBottom>
                                        Network Size Distribution by Method
                                    </Typography>
                                    <ResponsiveContainer width="100%" height={300}>
                                        <BarChart data={networkSizeData} layout="vertical" margin={{ left: 40, right: 20, top: 20, bottom: 20 }}>
                                            <CartesianGrid strokeDasharray="3 3" />
                                            <XAxis type="number" allowDecimals={false} />
                                            <YAxis dataKey="name" type="category" width={120} />
                                            <Tooltip formatter={(value) => value.toLocaleString()} />
                                            <Bar dataKey="value">
                                                {networkSizeData.map((entry, idx) => (
                            ...
                            <Card sx={{ height: '100%' }}>
                                <CardContent>
                                    ...
                                    <Typography variant="body2" color="text.secondary" paragraph>
                                        Exons with highest interaction counts and their respective protein
                                    </Typography>
