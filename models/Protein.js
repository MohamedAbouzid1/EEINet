...
class Protein {
  static async findById(proteinId) {
    ...
    if (/^\d+$/.test(proteinId)) {
      ...
      query = `
        ...
        WHERE p.protein_id = $1
      `;
      params = [parseInt(proteinId)];
    } else {
      // Otherwise, search by uniprot_id
      query = `
        SELECT 
          p.protein_id,
          p.uniprot_id,
  ...
  static async getExons(proteinId, limit = 100, offset = 0) {
    ...
    if (/^\d+$/.test(proteinId)) {
      ...
      query = `
        ...
        LIMIT $2 OFFSET $3
      `;
      params = [parseInt(proteinId), limit, offset];
    } else {
      // Otherwise, search by uniprot_id
      query = `
        SELECT DISTINCT
  ...
  static async getInteractions(proteinId, limit = 50, offset = 0, method = null) {
    ...
    if (/^\d+$/.test(proteinId)) {
      // Numeric: search by protein_id
      params = [parseInt(proteinId), parseInt(proteinId), limit, offset];
      if (method) {
      ...
      query = `
        ...
        WHERE (p1.protein_id = $1 OR p2.protein_id = $2)
        ${methodFilter}
        ORDER BY 
      ...
      // Otherwise, search by uniprot_id
      params = [proteinId, proteinId, limit, offset];
      if (method) {
        methodFilter = 'AND em.method_name = $5';
        params.push(method);
      }
