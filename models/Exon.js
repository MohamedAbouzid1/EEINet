...
class Exon {
  static async findById(exonId) {
    ...
    if (/^\d+$/.test(exonId)) {
      ...
      query = `
        ...
        WHERE e.exon_id = $1
      `;
      params = [parseInt(exonId)];
    } else {
      // Otherwise, search by ensembl_exon_id
      query = `
        SELECT 
          e.exon_id,
          e.ensembl_exon_id,
  ...
  static async getInteractions(exonId, limit = 50, offset = 0, method = null) {
    ...
    if (/^\d+$/.test(exonId)) {
      // Numeric: search by exon_id
      params = [parseInt(exonId), parseInt(exonId), limit, offset];
      if (method) {
      ...
      query = `
        ...
        WHERE (e1.exon_id = $1 OR e2.exon_id = $2)
        ${methodFilter}
        ORDER BY 
      ...
      // Otherwise, search by ensembl_exon_id
      params = [exonId, exonId, limit, offset];
      if (method) {
        methodFilter = 'AND em.method_name = $5';
        params.push(method);
      }
      query = `
        ...
        JOIN eei_methods em ON ei.method_id = em.method_id
        LEFT JOIN eei_orthology_mapping eom ON ei.eei_id = eom.eei_id
        WHERE (e1.ensembl_exon_id = $1 OR e2.ensembl_exon_id = $2)
        ${methodFilter}
        ORDER BY 
          CASE WHEN eom.confidence IS NOT NULL THEN eom.confidence ELSE 1.0 END DESC,
          ei.jaccard_percent DESC NULLS LAST
    ...
    console.log('getInteractions query:', query, 'params:', params);
    const result = await db.query(query, params);
    return result.rows;
  }
  ...
  static async getDetailedInteractions(exonId, limit = 50, offset = 0, method = null) {
    ...
    console.log('getDetailedInteractions query:', query, 'params:', params);
    const result = await db.query(query, params);
    return result.rows;
  }
  ...
  static async getInteractionCount(exonId, method = null) {
    ...
    let query, params;
    if (/^\d+$/.test(exonId)) {
      params = [parseInt(exonId), parseInt(exonId)];
      if (method) {
